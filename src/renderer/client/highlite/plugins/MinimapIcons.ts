...
export class MinimapIcons extends Plugin {
    ...
    private hasAction(npcOrObject: any, actionName: string): boolean {
        ...
        for (const action of actions) {
            if (
                (document as any).highlite.gameLookups.GameWorldActions[
                    action
    ...
    private updateNPCIcons(): void {
        ...
        try {
            const npcs = (document as any).highlite?.gameHooks?.EntityManager
                ?.Instance?._npcs;
            const mainPlayer = (document as any).highlite?.gameHooks
    ...
    private updateObjectIcons(): void {
        ...
        try {
            const objects = (document as any).highlite?.gameHooks
                ?.WorldEntityManager?.Instance?._worldEntities;
            const mainPlayer = (document as any).highlite?.gameHooks
                ?.EntityManager?.Instance?.MainPlayer;
