...
export class ChatItemTooltip extends Plugin {
    ...
    private scanChat() {
        const chatList: any[] = (
            document as any
        ).highlite?.gameHooks?.HR?.Manager?.getController()
        ...
        textNodes.forEach((tn) => {
            ...
            tn.data.replace(regex, (match, num: string, offset) => {
                ...
                try {
                    const itemDef = (
                        document as any
                    ).highlite.gameHooks.ItemDefMap.ItemDefMap.get(id);
    ...
    private updateDebug() {
        ...
        try {
            const inventoryItems = (document as any).highlite?.gameHooks
                ?.EntityManager?.Instance?.MainPlayer?.Inventory?.Items;
            if (inventoryItems) {
    ...
    private addItemToChatInput(itemId: number) {
        try {
            const chatController = (
                document as any
            ).highlite.gameHooks.HR.Manager.getController().ChatMenuController;
