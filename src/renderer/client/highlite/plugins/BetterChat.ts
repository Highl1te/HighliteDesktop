...
export class BetterChat extends Plugin {
    ...
    start(): void {
        ...
        this.log('Starting BetterChat');
        this.hideHsChat();
    }

    stop(): void {
        this.log('Stopping BetterChat');
        this.showHsChat();
    }
    ...
    hideHsChat(): void {
        const HTMLUIManager = this.gameHooks.HTMLUIManager.Instance;
        if (!HTMLUIManager) {
            return;
        }
        const screenMask = HTMLUIManager.getScreenMask();
        if (!screenMask) {
            return;
        }
        screenMask.getChatMenuQuadrant().getElement().style.display = 'none';
    }
    ...
    showHsChat(): void {
        ...
        screenMask.getChatMenuQuadrant().getElement().style.display = 'block';
    }
